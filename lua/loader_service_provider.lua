...
local n = function(t,d) pcall(function() s:SetCore("SendNotification",{Title="Service Provider",Text=t,Duration=d or 3}) end) end
...
if execQuality == "bad" or execQuality == "medium" then
    ...
    warn("[Service Provider] Executor UNC/sUNC quality is " .. execQuality .. ". Please change your executor.")
end
...
local base = "{{BASE_URL}}"
local apiUrl = base .. "/api/sp/execute?key=" .. ZUPER_KEY .. "&hwid=" .. hwid .. "&game_id=" .. universeId .. "&place_id=" .. placeId
...
if success then
    ...
    if func then
        ...
        if not ok then
            ...
            warn("Service Provider Runtime Error: " .. tostring(runtimeErr))
        end
    else
